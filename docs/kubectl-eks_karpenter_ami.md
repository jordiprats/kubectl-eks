...
Show AMI usage across Karpenter NodePools

### Synopsis

Show current AMIs in use per NodePool across clusters.

This helps identify which AMIs are being used by each NodePool for
inventory and tracking purposes.

```
kubectl-eks karpenter ami [flags]
```

### Examples

```
  # Show AMI usage for current cluster
  kubectl eks karpenter ami
