## kubectl-eks cache clear

Clear the local cluster cache

### Synopsis

Remove the local cache file (~/.kube/.kubectl-eks-cache). The next command that needs cluster data will fetch it from AWS.

```
kubectl-eks cache clear [flags]
```
